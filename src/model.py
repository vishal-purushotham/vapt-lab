...
# --- Layer Definitions (from modules.py) ---
...
class TemporalAttentionLayer(nn.Module):
    ...
    def _make_attention_input(self, v):
        ...
        combined = torch.cat((blocks_repeating, blocks_alternating), dim=2) # (b, N*N, 2*n_features)

        if self.use_gatv2:
            return combined.view(v.size(0), N, N, 2 * self.n_features)
        else:
...
# --- Main Model Definition (from mtad_gat.py) ---
...
class MTAD_GAT(nn.Module):
    ...
    def __init__(
        ...
        # Use classes defined within this file
